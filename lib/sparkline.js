...
export function sparklineDayLabels(buckets, locales = undefined) {
    ...
    const formatter = new Intl.DateTimeFormat(locales, points.length <= 5 ? {
        month: 'short',
        day: 'numeric',
        timeZone: 'UTC',
    } : {
    ...
    return points.map(point => ({
        date: point.date,
        label: formatter.format(new Date(point.ordinal * DAY_MS)),
        position: (point.ordinal - firstDay) / daySpan,
    }));
}
