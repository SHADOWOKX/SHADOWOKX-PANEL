...
- Verified Codex token activity with an account-side lifetime total and per-user locally accumulated daily history. The 48-pixel native Cairo chart appears only after two real local daily samples. The bounded curve cannot overshoot real points, every stored point is marked, the newest point is emphasized, and locale-aware labels preserve missing-day spacing.
...
Shadowokx Panel automatically locates Codex for the currently logged-in Linux user. No source edits or configured executable path are normally required. Detection checks, in order:

1. `codex` in the GNOME Shell process environment's `PATH`.
2. Standard current-user launch locations under the home and XDG data directories, including `~/.local/bin`, `~/bin`, npm, pnpm, Volta, Bun, asdf, Mise, and exported NVM/FNM locations.
3. The Codex binary bundled with the ChatGPT desktop application and standard system locations such as `/usr/local/bin`, `/usr/bin`, and `/snap/bin`.

Paths are assembled with GLib from the current process environment, `GLib.get_home_dir()`, and `GLib.get_user_data_dir()`; the extension never assumes that a home directory begins with `/home`. It launches the discovered executable as `codex app-server --stdio` using a fixed argument vector. Codex itself resolves the current user's authentication state. Shadowokx Panel does not search, parse, or copy Codex authentication/session files.

After initializing the bounded JSON-lines protocol, the extension requests only:
...
The optional usage-state icon uses completed days from Shadowokx Panel's per-user local history. It needs at least four completed days, compares the latest completed day with the mean of the preceding completed days, and deliberately excludes the still-changing current day. At 150% or more of that baseline it shows a small flame; at 50% or less it shows a quiet moon; otherwise it shows a neutral balanced state. The icon is omitted when history is missing, insufficient, or has no non-zero baseline. No pace is guessed from the remaining-limit percentage.

`account/usage/read` supplies optional account-side lifetime activity and real daily values. The lifetime total remains current Codex account information. The graph, local peak, peak day, and seven-day total are separate Shadowokx Panel history: on each successful refresh, only the validated current-day value is merged into that Linux user's local history. Earlier account-side buckets are not imported on first run.

There is intentionally no lifetime-counter delta calculation. Each local point uses the app-server's validated current-day value directly. Invalid dates and negative or non-integer values are discarded, same-day refreshes replace rather than duplicate that day's sample, missing dates remain real gaps, and stored history is bounded to seven actual days. A fresh installation begins with no previous history and can have at most one current-day point after its first successful refresh, so the chart honestly says “Not enough history yet” until another real day is collected.
...
- Codex limits, reset times, optional lifetime total, and the current day's validated token total are requested from the current user's local Codex app-server. Shadowokx Panel does not read, receive, or store OpenAI credentials.
- The last normalized Codex limit snapshot is cached in `${XDG_CACHE_HOME:-~/.cache}/shadow-panel/codex.json`. It does not contain graph history.
- The bounded per-user graph history is stored separately in `${XDG_DATA_HOME:-~/.local/share}/shadow-panel/codex-history.json`. A new installation does not ship or import another installation's history.
...
- Provider files are created with private permissions inside a private `shadow-panel` directory. Missing or malformed files safely produce an empty first-run state.
...
- Token activity can be unavailable for unsupported authentication modes or older Codex versions. The graph needs two locally accumulated daily samples, does not import prior account-side days or interpolate missing dates, and may therefore show fewer than seven points. Peak hour is not displayed because the provider exposes daily buckets, not hourly timestamps.
...
Confirm that Codex is installed and signed in. If it is available in your terminal:

```bash
codex --version
```

Codex may also be found automatically in supported current-user package-manager locations or bundled with the ChatGPT desktop application even when that terminal command is unavailable. Open Codex once and sign in, then use Refresh. The extension never requests an API key or reads Codex session files.
